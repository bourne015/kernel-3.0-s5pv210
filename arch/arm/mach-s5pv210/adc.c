...
//#define ADC_WITH_TOUCHSCREEN //modify by wanz
...
#ifdef ADC_WITH_TOUCHSCREEN //add by wanz
...
// yntan added start
int s3c_adc_get_value(int channel, int prescaler, int delay)
{
	int i = 5000;
	unsigned long ctrl, data, flags;

	spin_lock_irqsave(&adc_lock, flags);

	ctrl = S3C_ADCCON_PRSCEN | S3C_ADCCON_PRSCVL(prescaler & 0xff);
	...
	do {
		data = readl(base_addr + S3C_ADCCON);
	} while (i-- > 0 && !(data & S3C_ADCCON_ECFLG));

	data = readl(base_addr + S3C_ADCDAT0);
	...
	return (data & S3C_ADCDAT0_XPDATA_MASK_12BIT);
}
EXPORT_SYMBOL(s3c_adc_get_value);
// yntan added end
