...
static unsigned int wlan_on_gpio_table[][4] = {
        {GPIO_WLAN_EN , S3C_GPIO_OUTPUT, GPIO_LEVEL_NONE, S3C_GPIO_PULL_NONE},
        {GPIO_WLAN_HOST_WAKE, GPIO_WLAN_HOST_WAKE_AF,GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN},
};

static unsigned int wlan_off_gpio_table[][4] = {
        {GPIO_WLAN_HOST_WAKE, S3C_GPIO_INPUT, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN},
        {GPIO_WLAN_EN , S3C_GPIO_OUTPUT, GPIO_LEVEL_LOW, S3C_GPIO_PULL_DOWN},
};
...
static int brcm_wlan_power(int onoff)
{
        printk(KERN_INFO"------------------------------------------------");
        printk(KERN_INFO"------------------------------------------------\n");
        printk(KERN_INFO"MACHINE_IS_V70, %s Enter: power %s\n", __func__, onoff ? "on" : "off");
        if (onoff) {
		 ...
		 //s3c_gpio_slp_cfgpin(GPIO_WLAN_BT_EN, S3C_GPIO_SLP_OUT1);
		  ...
		  if (gpio_get_value(GPIO_BT_EN) == 0 || gpio_get_value(GPIO_BT_nRST) == 0) {
		  	s3c_gpio_setpull(GPIO_WLAN_BT_EN, S3C_GPIO_PULL_NONE);
			s3c_gpio_cfgpin(GPIO_WLAN_BT_EN, S3C_GPIO_OUTPUT);
...
int __exit brcm_wlan_exit(void)
{
        printk("%s: start\n", __func__);
...
module_init(brcm_wlan_init);
module_exit(brcm_wlan_exit);
