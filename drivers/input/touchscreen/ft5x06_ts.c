...
#if 0
#define ft5x06_ts_DEBUG	printk
#else
#define ft5x06_ts_DEBUG(...)
#endif

static struct i2c_client *this_client;
static struct ft5x0x_ts_platform_data *pdata;

struct keypos key_pos[] = {
	{50,110,801,870, KEY_MENU},
	{210,270,801,870, KEY_HOME},
	{370,430,801,870, KEY_BACK}
};
#define GTP_MAX_KEY_NUM  (sizeof(key_pos)/sizeof(key_pos[0]))
...
static void ft5x0x_ts_release(void)
{
	...
	ft5x06_ts_DEBUG("ft5x0x_ts_release\n");
	...
	if(event->touch_point)
	{
		if(event->y1 > 800 )
		{
			...
			for (i = 0; i < GTP_MAX_KEY_NUM; i++)
	       	{
	        		if(key_pos[i].minX <= event->x1 && key_pos[i].maxX >= event->x1 && key_pos[i].minY <= event->y1 && key_pos[i].maxY >= event->y1)
				{
					ft5x06_ts_DEBUG("key_pos[i].value = %d\n", key_pos[i].value);
	            			input_report_key(data->input_dev, key_pos[i].value, 1);
					pre_key |= (1<<i);
				}
	        	}
			
	        	event->touch_point = 0;
			ft5x06_ts_DEBUG("Key exist!");
		}
	}
	else if(pre_key)
    	{
	  	 for (i = 0; i < GTP_MAX_KEY_NUM; i++)
	      	{
	      		if(pre_key & (1<<i)){
	          		input_report_key(data->input_dev, key_pos[i].value, 0);
	  			pre_key &= (~(1<<i));
	  		}
	      	}
  	  	ft5x06_ts_DEBUG("Key Released!");
    	}
#ifdef CONFIG_FT5X0X_MULTITOUCH
	switch(event->touch_point) {
		case 5:
...
//    enable_irq(this_client->irq);
	...
	ft5x06_ts_DEBUG("ft5x0x_ts_pen_irq_work\n");
}
/***********************************************************************************************
Name	:	 
...
//    	disable_irq(this_client->irq);		
	ft5x06_ts_DEBUG("ft5x0x_ts_interrupt\n");
...
//	enable_irq(this_client->irq);
	...
	//msleep(290);
	//s3c_gpio_cfgpin(TP_INT, S3C_GPIO_SFN(4));
	s3c_gpio_setpull(TP_INT, S3C_GPIO_PULL_UP);
	enable_irq(IRQ_EINT(9));
#endif
}
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(gpio_request(TP_INT, "TP_INT")) {
		printk("TP_INT error!\n");
		return  -1;
       }
	
	s3c_gpio_cfgpin(TP_INT, S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(TP_INT, S3C_GPIO_PULL_UP);
	err = request_irq(IRQ_EINT(9), ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING | IRQF_DISABLED, "ft5x0x_ts", ft5x0x_ts);
	if (err < 0) {
